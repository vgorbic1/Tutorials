...
UNIQUE values must be unique for all rows in the table. Allows NULL values!
Example:
ALTER TABLE books ADD CONSTRAINT books_title_uk UNIQUE (title);

PRIMARY KEY uniquely identifies each row of the table. Does not allow NULL values! No need to put UNIQUE!
Examples:
...
FOREIGN KEY enforces referential integrity. Must match an existing value in the parent table or be NULL. Default is a restrict rule, which disallows the update or deletion of referenced data.
Examples:
ALTER TABLE orders 
...
CHECK specifies a condition that must be true. A single value can have multiple CHECK values
ALTER TABLE orders 
